...
@media (prefers-reduced-motion: reduce) {
  ...
  [data-flow], [data-drop], [data-sweep], [data-dash], [data-bob] {
...
/* ---- the figures come first -------------------------------------------
   The dashboard used to open on six stage rows, a findings box and a Run
   button; the numbers began below the fold. The run is how the figures are
   made, not what they say, so it moves under them. `order` on an existing
   grid does this without moving a line of markup -- and the DOM order is
   unchanged, so a screen reader and the tab key still meet the controls in
   the order they were written. */
/* Ordered by lifting the others, not by sinking the run panel: the runtime
   keeps `data-rise` on that div and discards any second bare attribute put
   beside it, so it cannot be given a hook of its own. Its three siblings
   already carry one -- `data-step` on each dashboard level, `data-nudge` on
   the prompt -- and negative orders raise them above it. Default order is 0,
   so the run panel lands last without being named at all. */
[data-step] { order: -2; }

/* The nudge that points down at it. It bobs, gently, once every two and a
   half seconds -- enough to catch the eye on a page of still figures,
   slow enough not to nag. */
@keyframes uiBob {
  0%, 62%, 100% { transform: translateY(0); }
  76%           { transform: translateY(4px); }
}
[data-bob] { animation: uiBob 2.5s ease-in-out infinite; }
[data-nudge] { order: -1; }
@media (hover: hover) and (pointer: fine) {
  [data-nudge]:hover { border-color: #C3D6CA; background: #FAFBFA; }
  [data-nudge]:hover [data-bob] { animation-duration: 1.1s; }
}
...
VALS = r"""
      ...
      progWindowNote: (() => {
        ...
        // "press Run now above" was written when the run panel was above.
        // It is under the figures now, and the prompt that sits directly
        // over it makes the offer better than a sentence up here can --
        // so this states the window and stops.
        return w + ' papers across ' + n + ' programmes, from the window the '
          + 'last run asked for.';
      })(),
      ...
      dashColleges: (() => {
        ...
        return COLLEGE_DATA.map((c, i) => ({
          ...
          full: c.name,
          color: c.color,
          icon: (window.__AAU && window.__AAU.collegeIcon ? window.__AAU.collegeIcon(c.name) : ''),
      ...
      // Names the window on screen, so the nudge is about THIS data rather
      // than a general invitation to press buttons.
      nudgeTitle: (() => {
        const y = (S_ && S_.years) || [];
        const w = y.length ? (y.length > 1 ? y[0] + '\u2013' + y[y.length - 1]
                              : String(y[0])) : 'this window';
        return 'These are the papers from ' + w + '.';
      })(),
      nudgeSub: 'Want another period, or to pick up work published since the '
        + 'last run? The run is just below \u2014 it takes about three minutes '
        + 'and the figures up here update themselves when it lands.',
      jumpToRun: () => {
        const n = document.querySelector('[data-nudge]');
        const el = n && n.parentElement
          && n.parentElement.querySelector('[data-rise]');
        if (!el) return;
      ...
      dashColIcon: (window.__AAU && window.__AAU.collegeIcon ? window.__AAU.collegeIcon(this.state.dashCol) : ''),
      dashColColor: (() => {
        const c = COLLEGE_DATA.find(x => x.name === this.state.dashCol);
